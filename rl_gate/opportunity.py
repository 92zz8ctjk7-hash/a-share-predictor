...
def _opportunity_one(code: str, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df[df["is_opportunity"] & df["label"].notna()][
        ["date", "code"] + FEATURES + ["label"]
    ].copy()
    out["y"] = (out["label"] > COST_PCT).astype(int)
    return out.dropna(subset=FEATURES).reset_index(drop=True)


def build_day_features(bars: pd.DataFrame) -> pd.DataFrame:
    """每日市场状态特征（gate 推理用，全日期、不做机会过滤）。"""
    df = bars.sort_values("date").reset_index(drop=True).copy()
    df = _add_features(df)
    return df[["date"] + FEATURES].dropna().reset_index(drop=True)


def build_opportunities(codes: List[str], before_date=None) -> pd.DataFrame:
    ...
    for code in codes:
        ...
        if bars is None or bars.empty:
            continue
        opp = _opportunity_one(code, bars)
